...
const Navbar = () => {
  ...
  const handleAuth = async () => {
    if (isSignup) {
      ...
      try {
        const response = await fetch("http://35.173.242.217:8000/api/signup", {
          method: "POST",
      ...
      try {
        const response = await fetch("http://35.173.242.217:8000/api/login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = await response.json();
        if (response.ok) {
          localStorage.setItem("user", data.user.username); // ✅ Save to localStorage
